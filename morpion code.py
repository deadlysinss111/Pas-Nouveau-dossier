...
def morpion():
  ...
  player=input(bcolors.HEADER+' O '+bcolors.ENDC+'or'+bcolors.OKBLUE+' X '+bcolors.ENDC+'?')
  ...
  if randint(0,1)==1:
    ...
    if player == 'X':
      column=int(input(bcolors.OKBLUE+"column ? (0, 1 ou 2)"))
      row=int(input(bcolors.OKBLUE+"row ? (0, 1 ou 2)"))
    if player == 'O':
      column=int(input(bcolors.HEADER+"column ? (0, 1 ou 2)"))
      row=int(input(bcolors.HEADER+"row ? (0, 1 ou 2)"))
  ...
  while isPlaying==1:
    if currentTurn==player:
      ...
      #si aucun jioueur n'est sur le point de gagner
      while endThisWhile==0:
        #on essaie des coups au hasard
