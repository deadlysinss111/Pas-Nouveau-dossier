...
def pve():
  ...
  if randint(0, 1) == 1:
    ...
    column = int(input(f"{textColor} column ? (0, 1 ou 2)"))
    row = int(input(f"{textColor} row ? (0, 1 ou 2)"))
...
def pvp():
  ...
  visuel(grid, "nothing")
  #on demande au joueur si il veux jouer X ou O
  player = input("player 1 : " + bcolors.HEADER + ' O ' + bcolors.ENDC + 'or' +
                 bcolors.OKBLUE + ' X ' + bcolors.ENDC + '?')
  assert player == 'O' or player == 'X'
  if player == 'O':
    playerTwo = 'X'
